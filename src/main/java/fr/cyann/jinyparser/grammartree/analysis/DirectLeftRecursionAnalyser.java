...
/**
 * The DirectLeftRecursionAnalyser definition. Detect when LR grammar style is
 * written and transform it to PEG compatible grammar.
 */
public class DirectLeftRecursionAnalyser implements GrammarTreeAnalyser {

    final static Logger logger = Logger.getLogger(DirectLeftRecursionAnalyser.class.getName());

	// region inner class
    /**
     * Gives all the unique infinite cycles; e.g. all the cycles present in the
     * graph that do not produce any lexem (in other words, all the cycles that
     * do not contains any GrammarLeaf element like "Word", "CharIn" etc.).<br>
     * Inspired from the Tarjan's Algorithms. But backtrack when a sub-rule that
     * create lexem was found.<br>
     * For uniqueness purpose, it needs a tree dictionary algorithm (see
     * oldBuildCyclesContext).
     *
        ...
        try {
            ...
            if (element instanceof GrammarNode) {
                ...
                for (GrammarElement child : ((GrammarNode) element)) {
                    ...
                    if (element instanceof Sequence || cycle != null) {
                        return cycle;
                    }
    ...
    private static boolean hasCycle(GrammarElement element, Recursive find) {
        if (element instanceof GrammarNode) {
            ...
            for (GrammarElement child : ((GrammarNode) element)) {
                ...
                if (element instanceof Sequence || result == true) {
                    return result;
                }
    ...
    public GrammarElement analyse(GrammarElement root) {
        ...
        while ((cycle = findCycles(root)) != null) {
            ...
            // logging
            logger.warning(String.format("Cycles detected: %s", cycle));
            ...
            if (!lastLink.containsKey(recursive)) {
                lastLink.put(recursive, recursive);
            }
